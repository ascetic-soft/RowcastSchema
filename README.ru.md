...
| PHP-атрибуты | директория (например, `src/Entity`) | Нет |

Формат определяется автоматически по пути `schema`:

- если `schema` указывает на директорию, используется парсер атрибутов
- если `schema` указывает на файл, парсер выбирается по расширению (`.php`, `.yaml`, `.yml`)

### Схема на атрибутах

Можно описывать схему напрямую в PHP-классах через атрибуты.
...
final class User
{
    ...
    #[Column]
    public UserStatus $status; // BackedEnum(string) -> type=enum, values из enum cases
}
...
Вывод типов для колонок через атрибуты:

- соответствие типов свойств: `int`, `string`, `bool`, `float`, `array`, `DateTimeInterface`
...
В файлах схемы можно указывать и строковые типы БД напрямую в `type`.
Неизвестные типы сохраняются как `databaseType` и попадают в SQL без преобразования.
...
$table->column('payload', 'jsonb'); // кастомный строковый тип БД
```
...
1. **Парсинг** — читает файл схемы (`.php` / `.yaml`) или сканирует директорию с атрибутами и строит внутреннюю модель `Schema`.
...
├── Attribute\
│   ├── Table, Column             # Метаданные сущности/таблицы и свойства/колонки
│   └── Index, ForeignKey         # Метаданные индексов и внешних ключей
├── Schema\
...
│   ├── AttributeSchemaParser         # Парсер директории с атрибутами
│   ├── ArraySchemaBuilder            # Общий билдер массив → Schema
│   ├── AttributeSchemaBuilder        # Билдер Reflection attributes → Schema
│   ├── ClassScanner                  # Сканер PHP-классов в директории
│   └── NamingStrategy                # Правила class/property -> table/column
├── Introspector\
